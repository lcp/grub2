...
protect_tpm2_export_nvindex (protect_args_t *args, void *data, int data_size)
{
  ...
  pub_info.nvPublic.attributes = TPMA_NV_POLICYWRITE | TPMA_NV_OWNERWRITE | TPMA_NV_OWNERREAD;
...
protect_tpm2_args_verify (protect_args_t *args)
{
  ...
  switch (args->action)
      ...
      if (args->tpm2_nvindex != 0)
	{
	  ...
	  if (!TPM_HT_IS_PERSISTENT (args->tpm2_nvindex) && TPM_HT_IS_NVINDEX (args->tpm2_nvindex) == false)
	    {
	...
	{
	  fprintf (stderr, N_("--tpm2-srk and --tpm2-nvindex are not specified when --action is 'remove'.\n"));
	  return GRUB_ERR_BAD_ARGUMENT;
	}
