				     ...
				     struct grub_tpm2_buffer *cmd_buf)
{
  ...
  if (cmd_buf->error != 0)
    return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("Failed to unmarshal the buffer for TPM2_PolicyAuthorize"));

  /* Retrieve Policy Digest */
  rc = TPM2_PolicyGetDigest (session, NULL, &pcr_policy, NULL);
  if (rc != TPM_RC_SUCCESS)
    return grub_error (GRUB_ERR_BAD_DEVICE, N_("Failed to get policy digest (TPM2_PolicyGetDigest: 0x%x)."), rc);
  ...
  if (sig_hash == TPM_ALG_NULL)
    return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("Failed to get the hash algorithm of the signature"));

  rc = TPM2_Hash (NULL, (TPM2B_MAX_BUFFER *)&pcr_policy, sig_hash,
		  TPM_RH_NULL, &pcr_policy_hash, NULL, NULL);
  if (rc != TPM_RC_SUCCESS)
    return grub_error (GRUB_ERR_BAD_DEVICE, N_("Failed to create PCR policy hash (TPM2_Hash: 0x%x)"), rc);

  /* Load the public key */
  rc = TPM2_LoadExternal (NULL, NULL, &pubkey, TPM_RH_OWNER,
			  &pubkey_handle, &pubname, NULL);
  if (rc != TPM_RC_SUCCESS)
    return grub_error (GRUB_ERR_BAD_DEVICE, N_("Failed to load public key (TPM2_LoadExternal: 0x%x)"), rc);
  ...
  if (rc != TPM_RC_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_DEVICE, N_("Failed to verify signature (TPM2_VerifySignature: 0x%x)"), rc);
      goto error;
    }
  ...
  if (rc != TPM_RC_SUCCESS)
    {
      err = grub_error (GRUB_ERR_BAD_DEVICE, N_("Failed to authorize PCR policy (TPM2_PolicyAuthorize: 0x%x)"), rc);
      goto error;
    }
