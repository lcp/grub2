...
tpm2_protector_dump_pcr (const TPM_ALG_ID_t bank)
{
  ...
  if (err != GRUB_ERR_NONE)
    {
      grub_printf ("Unsupported PCR bank [%s]\n", algo_name);
      return;
    }
			    ...
			    grub_uint8_t **key, grub_size_t *key_size)
{
  ...
  err = tpm2_protector_unseal_buffer (ctx, file_bytes, file_size, key, key_size);
...
static grub_err_t
tpm2_protector_unseal_persistent (const tpm2_protector_context_t *ctx, TPM_HANDLE_t sealed_handle,
				  grub_uint8_t **key, grub_size_t *key_size)
{
  tpm2key_policy_t policy_seq = NULL;
  bool dump_pcr = false;
...
static grub_err_t
tpm2_protector_unseal_nvindex (const tpm2_protector_context_t *ctx, TPM_HANDLE_t nvindex,
			       grub_uint8_t **key, grub_size_t *key_size)
{
  TPMS_AUTH_COMMAND_t authCmd = {0};
  ...
  return tpm2_protector_unseal_buffer (ctx, data.buffer, data_size, key, key_size);
}
...
tpm2_protector_check_args (tpm2_protector_context_t *ctx)
{
  ...
  if (ctx->mode == TPM2_PROTECTOR_MODE_SRK && ctx->nv != 0)
    return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("in SRK mode, a NV Index cannot be specified"));

  /* Checks for NV mode */
  if (ctx->mode == TPM2_PROTECTOR_MODE_NV && ctx->nv == 0)
    return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("in NV Index mode, a NV Index must be specified: --nvindex or -n"));
  ...
  if (ctx->mode == TPM2_PROTECTOR_MODE_NV && TPM_HT_IS_PERSISTENT (ctx->nv) == true &&
    ...
    return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("in NV Index mode with a persistent handle, a SRK cannot be specified"));

  if (ctx->mode == TPM2_PROTECTOR_MODE_NV &&
      (TPM_HT_IS_PERSISTENT (ctx->nv) == false && TPM_HT_IS_NVINDEX (ctx->nv) == false))
    return grub_error (GRUB_ERR_BAD_ARGUMENT, N_("in NV Index mode, a NV index must be a persistent or NV index handle"));
