  ...
  for (i = 0; i < TPM_MAX_PCRS; i++)
	...
	*next_pcr = '\0';
	...
	return grub_error (GRUB_ERR_BAD_NUMBER, N_("entry '%s' in PCR list is not a number"), current_pcr);
      ...
      if (pcr > TPM_MAX_PCRS)
	return grub_error (GRUB_ERR_OUT_OF_RANGE, N_("entry %llu in PCR list is too large to be a PCR number, PCR numbers range from 0 to %u"), (unsigned long long)pcr, TPM_MAX_PCRS);
